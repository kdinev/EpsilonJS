...
(function () {
	...
	Epsilon.ExpressionParser = function (expr) {
	...
	Epsilon.ExpressionTree = function (token, left, right) {
		switch (arguments.length) {
			...
			case 2:
				this.pointer = null;
				this.operator = token;
				this.left = new Epsilon.ExpressionTree(0);
				this.right = left;
				break;
			case 3:
				this.pointer = null;
		...
		this.getElementValue = function (el) {
			...
			if (el && el.getAttribute("data-formula")) {
				parser = new Epsilon.ExpressionParser(el.getAttribute("data-formula"));
				val = parser.evaluate();
			} else if (el) {
				val = el.innerText;
			}
	...
	Epsilon.epsilon = function (els) {
		var parser = new Epsilon.ExpressionParser(),
